...
class PESTResult:
    ...
    def to_dict(self) -> Dict:
        return {"political": self.political, "economic": self.economic,
            "social": self.social, "technological": self.technological, "summary": self.summary}
...
class SWOTResult:
    ...
    def to_dict(self) -> Dict:
        return {"strengths": self.strengths, "weaknesses": self.weaknesses,
            "opportunities": self.opportunities, "threats": self.threats, "summary": self.summary}
...
class Scenario:
    ...
    def to_dict(self) -> Dict:
        return {"name": self.name, "probability": self.probability,
            "target_price": self.target_price, "key_assumptions": self.key_assumptions,
...
class CausalChain:
    ...
    def to_dict(self) -> Dict:
        return {
            "cause": {"id": self.cause_event.id, "timestamp": self.cause_event.timestamp.isoformat(),
                "summary": self.cause_event.summary},
            "effect": {"id": self.effect_event.id, "timestamp": self.effect_event.timestamp.isoformat(),
                "summary": self.effect_event.summary},
            "time_gap": str(self.time_gap), "confidence": self.confidence, "description": self.description,
        }
...
class AnalyzerEngine:
    ...
    def analyze_pest(self, chapter_id: Optional[str] = None) -> PESTResult:
        ...
        for event in events:
            ...
            if any(t in tags for t in ["政策", "政治", "监管", "法规", "版号"]):
                result.political.append(summary)
            elif any(t in tags for t in ["经济", "市场", "收入", "营收", "增速"]):
                result.economic.append(summary)
            elif any(t in tags for t in ["社会", "用户", "玩家", "文化"]):
                result.social.append(summary)
            elif any(t in tags for t in ["技术", "AI", "科技", "创新"]):
                result.technological.append(summary)
    ...
    def analyze_swot(self, chapter_id: Optional[str] = None) -> SWOTResult:
        ...
        for event in events:
            tags = set(event.tags)
            summary = event.summary or str(event.data)[:100]
            if any(t in tags for t in ["优势", "增长", "利好", "成功", "突破"]):
                result.strengths.append(summary)
            elif any(t in tags for t in ["劣势", "亏损", "下降", "裁员", "风险"]):
                result.weaknesses.append(summary)
            elif any(t in tags for t in ["机会", "机遇", "趋势", "政策利好"]):
                result.opportunities.append(summary)
            elif any(t in tags for t in ["威胁", "竞争", "挑战", "压力"]):
                result.threats.append(summary)
    ...
    def compare_swot(self, chapter_a: str, chapter_b: str) -> Dict:
        ...
        return {
            "chapter_a": {"id": chapter_a, "swot": swot_a.to_dict()},
            "chapter_b": {"id": chapter_b, "swot": swot_b.to_dict()},
            "changes": {
                "new_strengths": [s for s in swot_b.strengths if s not in swot_a.strengths],
                "resolved_weaknesses": [w for w in swot_a.weaknesses if w not in swot_b.weaknesses],
                "new_opportunities": [o for o in swot_b.opportunities if o not in swot_a.opportunities],
                "new_threats": [t for t in swot_b.threats if t not in swot_a.threats],
            },
        }

    def analyze_scenario(self, optimistic_assumptions: List[str], optimistic_target: float,
        optimistic_prob: float, baseline_assumptions: List[str], baseline_target: float,
        baseline_prob: float, pessimistic_assumptions: List[str], pessimistic_target: float,
        pessimistic_prob: float) -> ScenarioAnalysis:
        analysis = ScenarioAnalysis(
            optimistic=Scenario(name="乐观情景", probability=optimistic_prob,
                target_price=optimistic_target, key_assumptions=optimistic_assumptions),
            baseline=Scenario(name="基准情景", probability=baseline_prob,
                target_price=baseline_target, key_assumptions=baseline_assumptions),
            pessimistic=Scenario(name="风险情景", probability=pessimistic_prob,
                target_price=pessimistic_target, key_assumptions=pessimistic_assumptions),
        )
        analysis.calculate_weighted_target()
        return analysis

    def find_causal_chains(self, min_confidence: float = 0.3) -> List[CausalChain]:
        events = self.timeline.timeline.get_all_events()
        chains = []
        for i in range(len(events)):
            for j in range(i + 1, len(events)):
                ...
                if confidence < min_confidence:
                    continue
                chain = CausalChain(cause_event=cause, effect_event=effect,
                    time_gap=effect.timestamp - cause.timestamp, confidence=confidence,
                    description=f"{cause.summary or '事件A'} → {effect.summary or '事件B'}")
                chains.append(chain)
        chains.sort(key=lambda x: x.confidence, reverse=True)
        return chains

    def _calculate_causality_confidence(self, cause: TimelineEvent, effect: TimelineEvent) -> float:
        confidence = 0.3
    ...
    def get_chapter_analysis(self, chapter_id: str) -> Dict:
        ...
        return {
            "chapter": {"id": chapter.id, "title": chapter.title,
                "duration": chapter.duration_label, "summary": chapter.summary},
            "event_count": event_count, "time_types": time_types,
    ...
    def generate_timeline_analysis(self) -> Dict:
        ...
        return {
            "overview": {"total_events": total_events, "chapter_count": chapter_count,
                "causal_chains_found": len(causal_chains),
                "top_causal_chains": [c.to_dict() for c in causal_chains[:5]]},
            "chapter_analyses": {c.id: self.get_chapter_analysis(c.id)
